# -*- coding: utf-8 -*-
"""
Created on Fri Oct 15 14:16:46 2021
...
#Input Variable Matrix Definition
    IntVar = np.array([[Plies, Relative_Density, AB_Ratio, HC_Ratio, AlSi12, Nylon, PLA, Ti6Al4V, BCC, BCCZ, OC, OT]])
...
#Return
    return [mass.item(), deform.item()], [AlSi12+Nylon+PLA+Ti6Al4V-1, BCC+BCCZ+OC+OT-1]
...
problem = Problem(12, 2, 2)
problem.types[:] = [igr13, Real(0.1,0.25), Real(0.7,1), Real(0.75,1.5), igr01, igr01, igr01, igr01, igr01, igr01, igr01, igr01]
problem.constraints[:] = "==0"
problem.function = massdeform

algorithm = NSGAII(problem, variator = CompoundOperator(SBX(), HUX(), BitFlip(), PM()))
...
#Display Prediction
pred_df = pd.DataFrame(np.reshape(Best_Opt, (1, len(Best_Opt))), columns=['Mass', 'Batt_Deform'], index=['Prediction'])
print(pred_df.T)
...
result_df = pd.DataFrame(np.reshape(Input_Param, (1,len(Input_Param))), columns=column_name, index=['Input Parameter'])
print(result_df.T)
